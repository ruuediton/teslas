  ...
  const [tasks, setTasks] = useState<Task[]>([
    {
      ...
      description: 'Acesse o app hoje para garantir seu bônus de presença.',
      reward: '150 Kz',
      status: 'available',
      type: 'daily',
      icon: 'calendar_today'
    ...
    {
      id: 'T2',
      name: 'Visualizar Fundo Rubi',
      description: 'Conheça os detalhes do Fundo Rubi para entender sua rentabilidade.',
      reward: '50 Kz',
      status: 'available',
      type: 'bonus',
      icon: 'visibility'
    ...
    {
      id: 'T3',
      name: 'Compartilhar DeepBank',
      description: 'Compartilhe o link do app com 3 amigos via WhatsApp.',
      reward: '1.000 Kz',
      status: 'available',
      type: 'special',
      icon: 'share'
    ...
    {
      id: 'T4',
      name: 'Concluir Introdução',
      description: 'Assista ao vídeo de introdução para novos investidores.',
      reward: '500 Kz',
      status: 'completed',
      type: 'daily',
      icon: 'play_circle'
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-12">
        ...
        <div className="bg-primary rounded-[32px] p-6 text-white shadow-xl shadow-primary/20 flex items-center gap-5 relative overflow-hidden">
           <div className="relative z-10 flex-1">
             <h2 className="text-xl font-black mb-1">{t.dailyGains}</h2>
        ...
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            tasks.map((task) => (
              <div 
                key={task.id}
                className={`bg-white dark:bg-dark-card rounded-[28px] border shadow-sm p-5 transition-all duration-300 ${
                  task.status === 'completed' ? 'border-gray-50 dark:border-white/5 opacity-60' : 'border-gray-100 dark:border-white/10 hover:border-primary/20'
                }`}
              >
                <div className="flex items-start gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                    task.status === 'completed' ? 'bg-gray-100 dark:bg-white/5 text-gray-400 dark:text-gray-600' : 'bg-primary/5 dark:bg-primary/20 text-primary'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded-md ${
                        task.type === 'daily' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-500' : 
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
              ...
              <div>
                <p className="text-dark dark:text-white font-extrabold">{t.noTasks}</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 max-w-[200px] mx-auto">{t.noTasksDesc}</p>
              </div>
            </div>
          )}
        </div>
